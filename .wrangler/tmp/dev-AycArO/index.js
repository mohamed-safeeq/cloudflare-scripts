...
// src/hubspot.js
var BASE_URL = "https://api.hubapi.com/crm/v3/objects/contacts";
async function getContactIdByPhone(phone, token) {
...
__name(getContactIdByPhone, "getContactIdByPhone");
async function updateReplyNeeded(contactId, value, token, propertyName) {
...
__name(updateReplyNeeded, "updateReplyNeeded");
...
// src/medon_pharmacy.js
var API_HEADERS = {
  apiKey: "68eb5895ccf654dbab7ceb2b",
  apiSecret: "9797111813f34277ac9f4c70120cc363",
  "Content-Type": "application/json"
};
var TEAM_1 = "68e37da971e717fc178a77f9";
var TEAM_2 = "68e9feacd5635bcce3bb9779";
...
__name(assignConversation, "assignConversation");
    ...
    try {
      ...
      if (attempt === 3) {
        ...
        if (channelId && contact?.name && contact?.phone?.length) {
          let ctaUrl = "https://medon.ae/branches-medon-pharmacy";
...
// src/Internal_Smartflow_Transcript_Zoho.js
var WAIT_MS = 7e4;
var MIN_VALID_AUDIO = 200;
...
var Internal_Smartflow_Transcript_Zoho_default = {
  async fetch(request, env) {
    try {
      ...
      const oauthURL = "https://accounts.zoho.in/oauth/v2/token?refresh_token=1000.a5a3b2eab5d90aa47156315f5ebb1bb8.caac58eb6431a787189a34284a669dde&client_id=1000.NQDGPN3F0OU2HJ8TVAO4TYVLX6B5EH&client_secret=6405cc9263714508df9a4f10270a997f6f3cc90d2f&grant_type=refresh_token";
...
// src/message_activity_log.ts
function getDirection(body) {
  return body.sender === body.contactId ? "customer_to_agent" : "agent_to_customer";
}
__name(getDirection, "getDirection");
function shouldSkip(body) {
  const type = body.whatsapp?.type;
  if (type === "template") {
    return { skip: true, reason: "template message" };
  }
  if (type === "interactive") {
    return { skip: true, reason: "interactive/bot message" };
  }
  return { skip: false };
}
__name(shouldSkip, "shouldSkip");
function buildLog(event, body) {
  ...
  return {
    ...
    customerPhone: direction === "customer_to_agent" ? wp.from ?? "" : wp.to ?? "",
    agentUserId: direction === "agent_to_customer" ? body.sender : null,
    contactId: body.contactId,
    contactName: body.contact?.name ?? "",
    messageType: wp.type,
    messageText: wp.text?.body ?? "",
    timestamp: wp.time,
    status: wp.status
  };
}
__name(buildLog, "buildLog");
var message_activity_log_default = {
  async fetch(request, env) {
    try {
      let body;
      try {
      ...
      const event = body.sender === body.contactId ? "message.received" : "message.sent";
      console.log(`[message_activity_log] event=${event} messageId=${body.id}`);
      const { skip, reason } = shouldSkip(body);
      if (skip) {
        console.log(`[message_activity_log] Skipped \u2014 ${reason}`);
        return new Response(
          JSON.stringify({ skipped: true, reason }),
          { status: 200, headers: { "Content-Type": "application/json" } }
        );
      }
      const log = buildLog(event, body);
      console.log(`[message_activity_log] direction=${log.direction} contact=${log.contactName} phone=${log.customerPhone}`);
      return new Response(
        JSON.stringify({ success: true, direction: log.direction, messageId: log.messageId, log }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error("[message_activity_log] ERROR:", msg);
      return new Response(
    ...
    try {
      switch (url.pathname) {
        ...
        case "/accountId/68a86975426f9f0608f849a2/medon": {
          return await medon_pharmacy_default.fetch(request, env);
        }
        // =======================================
        // 🔹3 SMARTFLOW TRANSCRIPT - INTERNAL 
        // =======================================
        case "/smartflow/zoho/internal": {
          return await Internal_Smartflow_Transcript_Zoho_default.fetch(request, env);
        }
        // ============================================
        // 🔹 4️⃣ MESSAGE ACTIVITY LOG
        // Endpoint to paste in your platform:
        //   POST https://<your-worker>.workers.dev/message/log?event=message.received
        //   POST https://<your-worker>.workers.dev/message/log?event=message.sent
        // ============================================
        case "/message/log": {
          const res = await message_activity_log_default.fetch(request, env);
...
// ../../Users/MohamedsafeeqS/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../Users/MohamedsafeeqS/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-E0vh5N/middleware-insertion-facade.js
...
// ../../Users/MohamedsafeeqS/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-E0vh5N/middleware-loader.entry.ts
